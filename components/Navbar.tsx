...
const NavBar = () => {
  ...
  return (
    <header
      className={`w-full text-white transition-all delay-75 ${
        isOpen ? "h-screen bg-blue-500 fixed md:h-14" : "h-14 absolute"
      } flex-col justify-around items-center  z-50 md:justify-between }`}>
      <div className="w-full h-14 items-center flex justify-between px-5 md:w-auto md:hidden">
        ...
        <span className={`${pathname === '/' ? 'flex' :'hidden'} md:hidden`}>
          {isOpen ? (
            <i
            ...
            <i
              className="mdi mdi-menu text-4xl"
              onClick={() => setIsOpen((prev) => !prev)}></i>
          )}
        </span>
      </div>
      ...
      <nav
        ...
        <ul
          ...
          {navLinks.map((item) => (
            <li
              className="w-full h-16 text-center transition-all delay-100 hover:text-blue-100 hover:scale-110 md:h-auto md:px-1"
              key={item.title}
